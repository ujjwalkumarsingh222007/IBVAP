...
class AIService:
    ...
    def process_frame(
        ...
        # 2b. Face Recognition & Vehicle Registry Lookup
        ...
        for det in raw_detections:
            ...
            # --- Person Processing with Face Recognition & In-Memory Cache ---
            if cls_name == "person":
            ...
            # --- Vehicle Processing with License Plate Registry Lookup ---
                ...
                is_watchlist = bool(det.get("watchlist_match", False))
                if is_watchlist:
                    det["is_known"] = False
                    det["is_flagged"] = True
                    det["status"] = "FLAGGED"
        ...
        # Synchronize active tracks to expire any disappeared tracks immediately
        ...
        # 6. Run Member 2 ANPR Pipeline (skipped only when frame contains exclusively persons on non-ANPR cameras)
        is_person_only = (len(raw_detections) > 0 and all(d.get("class_name") == "person" for d in raw_detections))
        is_anpr_camera = any(tag in camera_id.upper() for tag in ("ANPR", "GATE", "VEHICLE", "ENTRY", "EXIT", "TOLL"))
        should_run_anpr = is_anpr_camera or not is_person_only

        if self.anpr_pipeline is not None and should_run_anpr:
            try:
        ...
        logger.info(
            "AI FRAME PERF | Total: %.1fms | CV Track: %.1fms | Face: %.1fms | Detections: %d",
            total_ai_ms,
            t_cv_ms,
            t_face_ms,
            len(raw_detections),
