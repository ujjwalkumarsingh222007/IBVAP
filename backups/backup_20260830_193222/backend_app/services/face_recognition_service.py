...
class FaceRecognitionService:
    ...
    def _populate_cache(self, db: Session) -> None:
        ...
        # Determine target dimension based on active descriptor engine
        target_dim = 512 if (self._insightface_enabled and self._insightface_app is not None) else 1306
        ...
        logger.info(
            "Face recognition in-memory cache initialized with %d %d-D embeddings across %d persons.",
            len(self._cache_metadata),
            target_dim,
            len(people),
        )
    ...
    def _detect_faces_cascade(
        ...
        """Haar Cascade multi-angle face detection fallback."""
        ...
        if not self.frontal_alt2.empty():
            f_alt2 = self.frontal_alt2.detectMultiScale(
                gray, scaleFactor=1.15, minNeighbors=3, minSize=min_size, flags=cv2.CASCADE_SCALE_IMAGE
            )
        ...
        if not self.profile_cascade.empty():
            ...
            p_right = self.profile_cascade.detectMultiScale(
                flipped_gray, scaleFactor=1.15, minNeighbors=3, minSize=min_size, flags=cv2.CASCADE_SCALE_IMAGE
            )
    ...
    def validate_registration_face(
        ...
        """
        Perform strict face quality checks for guided registration:
        - Exactly 1 face in view
        - Face minimum resolution
        - Blur check (Laplacian variance)
        - Brightness / lighting check
        - Position / center guide check
        """
        if image_bgr is None or image_bgr.size == 0:
        ...
        if h_img < 48 or w_img < 48:
            return False, "Image resolution too low.", None

        faces = self.detect_faces(image_bgr, min_size=(35, 35))
        if len(faces) == 0:
            return False, "No face detected. Look directly at the camera.", None
        if len(faces) > 1:
            return False, "Only one person should be visible in camera.", None

        fx, fy, fw, fh = faces[0]

        # 1. Face Size Check
        if fw < 45 or fh < 45:
            return False, "Move closer to the camera.", faces[0]
        ...
        if face_crop.size > 0:
            ...
            if mean_val < 35.0:
                return False, "Improve lighting (image too dark).", faces[0]
            if mean_val > 240.0:
                return False, "Lighting is too bright / washed out.", faces[0]

            # 3. Blur Check (Laplacian Variance)
            lap_var = float(cv2.Laplacian(face_crop, cv2.CV_64F).var())
            if lap_var < 30.0 and (fw >= 80 and fh >= 80):
                return False, "Hold still (image blurry).", faces[0]
    ...
    def get_face_crop(
        ...
        if face_bbox is not None:
            ...
            detected = self.detect_faces(image_bgr, min_size=(20, 20))
            if detected:
                fx, fy, fw, fh = detected[0]
            elif h_img >= 50 and w_img >= 25:
                head_h = int(h_img * 0.60)
                head_crop = image_bgr[0:head_h, :]
                detected_head = self.detect_faces(head_crop, min_size=(18, 18))
                if detected_head:
                    fx, fy, fw, fh = detected_head[0]
                else:
                    aspect = w_img / float(h_img)
                    if 0.6 <= aspect <= 1.4:
                        fx, fy, fw, fh = 0, 0, w_img, h_img
                    else:
                        return None
    ...
    def check_duplicate_registration(
        self, new_embedding: List[float], threshold: float = 0.68, db: Optional[Session] = None
    ...
    def _get_or_create_track(
        ...
        # Untracked person instance — assign ephemeral isolated track ID
        t_id = int(now * 1000000 + random.randint(100, 999)) % 10000000
        new_key = f"{camera_id}:ephemeral_{t_id}"
        new_track = TrackIdentityState(
            track_id=t_id,
            camera_id=camera_id,
            first_seen=now,
    ...
    def process_person_detection(
        ...
        # Run immediately on frame 1 / PENDING, or when cooldown expires (~650ms)
        ...
        if should_run_recognition:
            ...
            # State Transition with Multi-Frame Confirmation & Anti-Bleed Protection
            if instant_match is not None:
                ...
                if matched_person.status == "KNOWN":
                    ...
                    # Confirm KNOWN on high similarity or 2 consistent frame matches
                    if sim >= 0.82 or track.known_streak >= self.known_confirmation_frames:
                    ...
                    if sim >= 0.82 or track.flagged_streak >= self.known_confirmation_frames:
                ...
                if track.confirmed_status in ("KNOWN", "FLAGGED"):
                    # If a face was detected and failed matching, degrade immediately
                    if face_detected:
                        track.confirmed_status = "UNKNOWN"
                        track.confirmed_person = None
                        track.confirmed_similarity = 0.0
                    elif track.missed_frames > self.grace_period_frames:
                        track.confirmed_status = "UNKNOWN"
                        track.confirmed_person = None
                        track.confirmed_similarity = 0.0
                else:
        ...
        if should_run_recognition:
            logger.info(
                "PERF | Track: %s | Match: %s (%.2f) | State: %s | emb: %.1fms, match: %.2fms | total: %.1fms",
                track.track_id,
                match_name,
                sim_val,
                final_status,
