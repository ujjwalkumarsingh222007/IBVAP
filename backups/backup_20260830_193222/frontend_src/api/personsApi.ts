...
export const personsApi = {
  ...
  registerMultiSamplePerson: async (
    name: string,
    status: 'KNOWN' | 'FLAGGED',
    samples: { blob: Blob; angle: string }[],
    notes?: string,
    ...
    samples.forEach((sample, i) => {
      formData.append('files', sample.blob, `sample_${i}_${sample.angle}.jpg`);
      formData.append('angles', sample.angle);
    });
    if (samples.length > 0) {
      formData.append('file', samples[0].blob, 'face.jpg');
    }
